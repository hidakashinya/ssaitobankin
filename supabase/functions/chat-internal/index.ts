...
const DIFY_API_KEY = Deno.env.get('DIFY_API_KEY_INTERNAL') 
                  ?? Deno.env.get('DIFY_API_KEY_CUSTOMER') 
                  ?? Deno.env.get('DIFY_API_KEY') 
                  ?? '';

if (!DIFY_API_KEY) {
  throw new Error('No DIFY API KEY found (tried INTERNAL, CUSTOMER, default)');
}

// デバッグ用ログ
console.log('Using API key from:', 
  Deno.env.get('DIFY_API_KEY_INTERNAL') ? 'INTERNAL' : 
  Deno.env.get('DIFY_API_KEY_CUSTOMER') ? 'CUSTOMER' : 
  Deno.env.get('DIFY_API_KEY') ? 'DEFAULT' : 'NONE'
);
